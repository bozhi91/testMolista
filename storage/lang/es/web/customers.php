<?php
	return [
		'login.title' => 'Acceso usuarios',
		'login.email' => 'Email',
		'login.password' => 'Password',
		'login.button' => 'Login',
		'login.error' => 'Los datos no concuerdan con nuestro registro.',

		'register.title' => 'Registro usuarios',
		'register.name.first' => 'Nombre',
		'register.name.last' => 'Apellido',
		'register.email' => 'Email',
		'register.email.used' => 'Este email ya se encuentra en uso',
		'register.password' => 'Password',
		'register.phone' => 'Teléfono',
		'register.button' => 'Registrarse',
		'register.success' => '<h4>Felicitaciones</h4><p>El registro se ha completado correctamente.</p>',
	];